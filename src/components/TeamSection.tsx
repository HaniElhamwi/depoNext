...
// components/about/TeamSection.tsx (Server Component – NO 'use client')
...
const TeamSection = async () => {
  ...
  try {
    ...
    teamMembers = res?.data?.map((item: any) => ({
      id: item.id,
      name: item.name,
      role: item.position,
      bio: item.bio?.[0]?.children?.[0]?.text || '',
          ...
          {teamMembers.map((member) => (
            <Card key={member.id} className="hover-effect">
              <CardContent className="p-0">
                ...
                <div className="p-6">
                  <h3 className="text-xl font-bold mb-1">{member.name}</h3>
                  <p className="text-ssu-blue font-medium mb-3">{member.role}</p>
                  <p className="text-gray-600">{member.bio}</p>
                </div>
              </CardContent>
            </Card>
